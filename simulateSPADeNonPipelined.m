function [time,time_u,yL,df,MSE,ST] = simulateSPADeNonPipelined(h,tauSystemScenarios,phi,Gamma,C_aug,K,F,pattern,SIMULATION_TIME,fh)
% SIMULATESPADENONPIPELINED - A function to simulate the SPADe design in Matlab for non-pipelined implementation
% Arguments:
...
initialStateDisturbance=-0.03; %HARDCODED to z0(3) in line 42
...
%%%  END -- Simulate Square Reference======================================
    ...
    time_u{loop}=timeU;
    df{loop}=u;
    mse_r{loop}=reference*ones(1,length(yL{loop})); %IN THIS CASE, REFERENCE=0
    %% Compute MSE
